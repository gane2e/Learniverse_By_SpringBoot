...
### ✔회원가입
- **[아이디 중복체크]**
   1. 아이디 입력 후 "중복확인" 클릭 시 사용자가 입력 한 ID를 JSON형태(key,Value)로 FetchAPI 통신을 POST 방식으로 요청 <br />
   2. 서버에서 아이디 중복체크 후 반환된 "true", "false" 값으로 중복 여부에따른 success 로직을 실행
- **[유효성검사]** <br />
  1. 회원가입 버튼 클릭 시 자바스크립트의 유효성검사 함수를 실행<br />
  2. 각 입력폼이 빈 문자열인지 또는 형식에 맞지않는지 검사<br />
<img src="https://github.com/user-attachments/assets/9625332e-4a5d-49c5-9ed0-10d0f6ff3d8d" width="800px"></br>
### ✔로그인
- **[입력필드 필수값]** <br />
   1. 아이디 및 비밀번호는 html태그 속성 중 required를 사용하여 "폼 제출 전 반드시 채워져있어야 하는 필드"임을 명시
- **[아이디/비밀번호 체킹]** <br />
   1. 스프링 시큐리티를 통해 로그인처리
<img src="https://github.com/user-attachments/assets/003fcb9f-c72e-4bb8-8fdb-03c8bd63924e" width="800px"><br />
### ✔수강신청
- 검색기를 통해 카테고리 및 키워드 검색이 가능하며, 회원일 시 수강신청이 가능합니다.
- 교육 상세페이지에서 해당 교육과정의 영상 미리보기가 가능합니다. (진도율 저장X)
<img src="https://github.com/user-attachments/assets/579a5b9a-0d7e-4724-b6ef-999fde0a30c5" width="800px"> <br />

### ✔영상 학습하기
- 해당 교육과정에 등록 된 영상 목록을 조회할 수 있으며, 영상은 1번부터 순차적으로 이수할 수 있습니다.
- 조회중인 영상을 100% 시청하지 않았을 시 다음영상 실행 불가 / 총 이수율이 100%가 아닐 시 시험보기가 불가합니다. 
